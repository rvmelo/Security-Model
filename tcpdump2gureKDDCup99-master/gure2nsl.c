	...
	if(fp_write == NULL)
		...
		while(*sep != 0){
			...
			if(*sep == ' '){
				...
				if(blank_num == 37){//dst_host_count
				...
				if(blank_num == 38){//dst_host_srv_count
...
int main(int argc, char *argv[])
{
	...
	if(argc < 2){
		...
		printf("error: missing pattern class!!!\n");
		return 0;

	}	
		
	generate_nsl_data("trafAld.list", "dataset.txt", argv[1]);
